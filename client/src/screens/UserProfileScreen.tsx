...
type UserProfileScreenProps = StackScreenProps<DiscoverStackParamList, 'UserProfile'>;

const isProfileRecord = (
  value: UserProfilePreview | Partial<ProfileRecord> | undefined,
): value is Partial<ProfileRecord> => {
  return Boolean(value && ('full_name' in value || 'avatar_url' in value));
};
...
const UserProfileScreen = ({ route, navigation }: UserProfileScreenProps): React.JSX.Element => {
  ...
  const user = {
    name: previewUser?.name || profileUser?.full_name || 'Kullanıcı',
    username: previewUser?.username || profileUser?.username || 'kullanici',
    avatar: previewUser?.avatar || profileUser?.avatar_url || 'https://i.pravatar.cc/150',
