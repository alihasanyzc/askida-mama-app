...
import type { DiscoverStackParamList, ProfileStackParamList } from '../types/navigation';
import type { UserProfilePreview } from '../types/domain';

type FollowersScreenProps =
  | StackScreenProps<DiscoverStackParamList, 'Followers'>
  | StackScreenProps<ProfileStackParamList, 'Followers'>;
...
const FollowersScreen = ({ route, navigation }: FollowersScreenProps): React.JSX.Element => {
  ...
  const handleUserPress = (user: FollowUser) => {
    const userPreview: UserProfilePreview = {
      ...user,
    ...
    const discoverNavigation = navigation as StackNavigationProp<DiscoverStackParamList>;
    discoverNavigation.push('UserProfile', { user: userPreview });
  };

  const renderUser: ListRenderItem<FollowUser> = ({ item }) => {
